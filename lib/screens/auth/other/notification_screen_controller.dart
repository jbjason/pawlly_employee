...
class NotificationScreenController extends GetxController {
  ...
  void init({bool showLoader = true}) {
    ...
    getNotifications(AuthService.getNotificationDetail(
      page: page,
  ...
  Future<void> removeNotification({required BuildContext context, required String notificationId}) async {
    showConfirmDialogCustom(
      ...
      onAccept: (ctx) async {
        isLoading(true);
        await AuthService.removeNotification(notificationId: notificationId).then((value) {
          init();
  ...
  Future<void> clearAllNotification({required BuildContext context}) async {
    showConfirmDialogCustom(
      ...
      onAccept: (ctx) async {
        isLoading(true);
        await AuthService.clearAllNotification().then((value) {
          init();
        }).catchError((error) {
          toast(error.toString());
        }).whenComplete(() => isLoading(false));
