...
import 'package:pawlly_employee/utils/local_storage.dart';
...
import '../main.dart';
import '../screens/auth/sign_in_controller.dart';
...
void ifNotTester(VoidCallback callback) {
  if (!SignInController().selectRole.map((e) => e.email).toList().contains(loginUserData.value.email)) {
    callback.call();
  } else {
  ...
  showInDialog(
    ...
    builder: (_) {
      return WillPopScope(
          onWillPop: () {
            return Future(
              () => currentAppVersionCode >= appConfigs.value.employeeMinimumForceUpdateCode,
            );
          },
